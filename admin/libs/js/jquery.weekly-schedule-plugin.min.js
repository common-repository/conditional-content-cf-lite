(function ($) {
  ...
  DayScheduleSelector.prototype.renderRows = function () {
    ...
    $.each(generateDates(start, end, interval), function (i, d) {
      var daysInARow = $.map(new Array(days.length), function (_, i) {
        return '<td class="time-slot" title="' + ifsohhmm(d) + '" data-time="' + hhmm(d) + '" data-day="' + days[i] + '"></td>'
      }).join();

      var hmmAmPmD = hmmAmPm(d);
      var hour = parseInt(hmmAmPmD.split(":")[0]);

      var displayT = (hmmAmPmD != "0:00" && hour % 3 == 0) ? hmmAmPmD : "";
      if (hour == 0) displayT = "00:" + hmmAmPmD.split(":")[1];

      $el.append('<tr><td class="time-label">' + displayT + '</td>' + daysInARow + '</tr>');
    });
  };
  ...
  function ifsohhmm(date) {
    var _hhmm = hhmm(date);
    var _fromHours = date.getHours();
    var date2 = new Date(date.getTime() + 3600000); // add 1 hour
    var _secondhhmm = hhmm(date2);

    return _hhmm + " - " + _secondhhmm;
  }
